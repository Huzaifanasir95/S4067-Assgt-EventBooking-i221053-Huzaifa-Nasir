...
const EventsPage = ({ events, setEvents }) => {
  ...
  const fetchBookings = async () => {
    try {
      ...
      for (const uid of userIds) {
        if (!names[uid]) {
          const userResponse = await axios.get(`${API_BASE_URL}/users/${uid}`);
          names[uid] = userResponse.data.name || 'Unknown';
        }
      }
      for (const eid of eventIds) {
        if (!titles[eid]) {
          const eventResponse = await axios.get(`${EVENT_SERVICE_URL}/events/${eid}`);
  ...
  const handleAddEventSubmit = async (e) => {
    e.preventDefault();
    try {
      const response = await axios.post(`${EVENT_SERVICE_URL}/events`, {
      ...
      const updatedEvents = await axios.get(`${EVENT_SERVICE_URL}/events`);
      setEvents(updatedEvents.data); // Update the events in App.js
      setNewEvent({
        title: '',
        description: '',
        date: '',
  ...
  const handleBookNow = (eventId) => {
    ...
    console.log(`Attempting to navigate to /book/${eventId}`); // Debug log
    navigate(`/book/${eventId}`, { replace: false }); // Ensure navigation happens
  };
